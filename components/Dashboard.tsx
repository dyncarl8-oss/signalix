...
import { Activity, RefreshCw, Zap, LogOut, ChevronDown, Crown, CreditCard, UserCircle2, Terminal, Sparkles, Sidebar as SidebarIcon } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-[#050508] text-gray-200 font-sans selection:bg-purple-500/30 selection:text-white overflow-hidden">
      ...
      <div className="hidden lg:block h-full">
         <Sidebar 
           selectedPair={selectedPair || { symbol: '', base: '', quote: '', name: '', type: 'CRYPTO' }}
           onSelectPair={handlePairSelect}
           selectedTimeframe={selectedTimeframe || ''}
           onSelectTimeframe={handleTimeframeSelect}
           credits={credits}
           onAnalyze={() => {}} // Disabled in this new flow, action is implicit on click
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          <div className="w-80 h-full bg-[#050508] border-r border-gray-800 shadow-2xl animate-in slide-in-from-left duration-200">
             <Sidebar 
               selectedPair={selectedPair || { symbol: '', base: '', quote: '', name: '', type: 'CRYPTO' }}
               onSelectPair={(p) => { handlePairSelect(p); setIsSidebarOpen(false); }}
               selectedTimeframe={selectedTimeframe || ''}
               onSelectTimeframe={(t) => { handleTimeframeSelect(t); setIsSidebarOpen(false); }}
               credits={credits}
               onAnalyze={() => {}} 
               isAnalyzing={sessionState === 'analyzing'}
      ...
      <div className="flex-1 flex flex-col h-full relative w-full">
        {/* Header */}
        <header className="h-16 flex items-center justify-between px-4 md:px-6 border-b border-gray-900 bg-[#050508] relative z-20 shrink-0">
          <div className="flex items-center gap-4">
             {/* Mobile Menu Toggle */}
             <button 
               className="lg:hidden p-2 text-gray-400 hover:text-white border border-gray-800 rounded-md"
          ...
          <div className="flex items-center gap-4">
             {user.isPro ? (
               <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-yellow-900/40 to-yellow-600/10 border border-yellow-700/50">
        ...
        {/* Main Feed Area */}
        <main className="flex-1 overflow-y-auto p-3 md:p-6 relative scroll-smooth w-full">
          {/* Background Ambient */}
